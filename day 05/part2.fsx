...
let seeds =
    ...
    |> Seq.map uint64
    |> Seq.chunkBySize 2
    |> Seq.collect (fun [| start; length |] -> [ start .. (start + length - 1UL) ])

let soils = seeds |> Seq.map lookupchain
#time
let solution = soils |> Seq.min
//Real: 00:15:46.551, CPU: 00:29:51.812, GC gen0: 13586, gen1: 15, gen2: 8
//72263011UL
let run () =
    printf "Testing.."
